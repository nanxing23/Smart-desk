...
AudioRecorder::AudioRecorder(QWidget *parent)/*初始化设置*/
{
    Q_UNUSED(parent);/*用于标记在函数体内部未使用的参数，以避免编译器产生关于未使用参数的警告。*/
    ...
    /* 信号槽连接，更新录音level显示 */
    connect(m_probe, &QAudioProbe::audioBufferProbed,//当QAudioProbe检测到新的音频缓冲区数据时，它会发射这个信号
...
void AudioRecorder::startRecorder()/* 开始录音槽函数 */
{
    ...
    if (m_audioRecorder->state() == QMediaRecorder::StoppedState) {
        ...
        settings.setCodec(codecsVar.at(0).toString());/* 音频编码 audio/x-raw, layout=(string)interleaved*/
        settings.setSampleRate(sampleRateVar[2].toInt());/* 采样率 16000*/
        settings.setBitRate(bitratesVar[0].toInt());/* 比特率 0*/
        settings.setChannelCount(channelsVar[1].toInt());/* 通道 1*/
        ...
        QString container = containersVar.at(20).toString();/* 容器/支持的格式 */
...
void AudioRecorder::processBuffer(const QAudioBuffer& buffer)/*用于处理音频缓冲区*/
{
